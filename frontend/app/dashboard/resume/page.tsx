...
export default function ResumePage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch("http://localhost:8000/api/analyze-resume", { method: "POST", body: formData });
  ...
  const handleOptimize = async () => {
    ...
    try {
      ...
      const res = await fetch("http://localhost:8000/api/optimize-resume", { method: "POST", body: formData });
      if (!res.ok) throw new Error();
