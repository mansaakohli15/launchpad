...
export default function InterviewPage() {
  ...
  const startInterview = async () => {
    ...
    setEvaluation(null);
    try {
      const res = await fetch("http://localhost:8000/api/interview/question", {
        method: "POST",
  ...
  const submitAnswer = async () => {
    ...
    try {
      const evalRes = await fetch("http://localhost:8000/api/interview/evaluate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (qNum < TOTAL_QUESTIONS) {
        const nextQ = qNum + 1;
        const qRes = await fetch("http://localhost:8000/api/interview/question", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
